...
// If isActive = false, we don’t track time or run screenshots
let isActive = true;
...
// Capture screenshot + send to backend for analysis
async function captureAndAnalyze() {
  ...
  try {
    ...
    if (!tab) {
      console.log('No active tab found, skipping screenshot.');
      return;
    }

    // Skip chrome:// or extension pages
    if (tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://')) {
      console.log('Skipping screenshot for internal URL:', tab.url);
      return;
    }
    ...
    chrome.tabs.captureVisibleTab(null, { format: 'jpeg', quality: 50 }, async (dataUrl) => {
      ...
      // Send screenshot data to backend
      try {
        ...
        chrome.storage.local.get(['productivityStats'], (data) => {
          ...
          if (result.unproductive) {
            ...
            chrome.notifications.create('', {
              ...
              message: result.message || 'Current activity seems unproductive.',
              iconUrl: 'icon48.png',
              type: 'basic'
            });
          } else {
    ...
    console.error('Error during captureAndAnalyze:', err);
  }
}

// One initial screenshot after 5s on startup
